...
class CoagentA2C(Coagent):
    '''
    This Agent does not use a critic as of now
    '''
    def __init__(self, params):
        super(CoagentA2C, self).__init__(params)
    ...
    def train(self, batch_x, batch_y):
        ...
        for i in range(self.network.num_coagent_layers() - 1, -1, -1):
            ...
            ### ###
            with torch.no_grad():
                if i == self.network.num_coagent_layers() - 1: # output layer 
                    reward = delta
                if i != self.network.num_coagent_layers() - 1: # input and hidden layers 
                    reward = critic_target - critic_output
            ### ### 
