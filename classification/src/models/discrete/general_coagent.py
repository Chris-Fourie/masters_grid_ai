...
class GeneralCoagent(nn.Module):
    def __init__(self, params):
        ...
        for i in range(self.num_layers + 1):
            if i == 0:
                self.layers.append(nn.Linear(self.in_features[0], self.num_coagents *2 )) # Input layer 

            elif i == self.num_layers :
                self.layers.append(nn.Linear(self.num_coagents , self.num_classes)) # Output layer 
            else:
                self.layers.append(nn.Linear(self.num_coagents, self.num_coagents * 2)) # Hidden layer 
    ...
    def get_forward_softmax( self, model , x):
        return f.softmax(model(x).reshape([-1, self.num_coagents, 2]), dim = 2) # TODO ask Shahil # how does passing inputs through a single layered NN generate a sensible outcome and what are we updating when we update the policy of these nodes? 
    ...
    def sample_all_coagent_states(self, x, greedy = False):
        ...
        for i in range(self.num_layers):
            # print(f'XXX {self.layers[0]}') # TEMP # Linear(in_features=784, out_features=128, bias=True)
            # print(f'XXX {self.layers[1]}') # TEMP # Linear(in_features=64, out_features=128, bias=True)
            # print(f'XXX {self.layers[2]}') # TEMP # Linear(in_features=64, out_features=128, bias=True)
            state_softmax_probs = self.get_forward_softmax(self.layers[i], state)
            # ic(f'XXX {state_softmax_probs.shape}') # TEMP

            # print(f'XXX {state_softmax_probs.shape}') # TEMP # 50000 x 64 x 2
            self.all_softmax.append(state_softmax_probs)
            if greedy :
                state = state_softmax_probs.max(dim = 2)[1].float()
                # ic(f'XXX {state.shape}') # TEMP 
                # ic(f'XXX {state}') # TEMP
                # assert 0 # TEMP 
            else:
